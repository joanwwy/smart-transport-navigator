...
export const GlobalNotificationBar: React.FC<GlobalNotificationBarProps> = ({
  ...
  const [incidents, setIncidents] = useState<GlobalIncident[]>(DEFAULT_INCIDENTS);
  ...
  // Poll backend LTA traffic incidents & train alerts
  useEffect(() => {
    async function loadLtaLiveIncidents() {
      try {
        ...
        const newIncidents: GlobalIncident[] = [];

        if (trafficRes && trafficRes.ok) {
          const trafficData = await trafficRes.json();
          if (trafficData.value && Array.isArray(trafficData.value)) {
            trafficData.value.slice(0, 4).forEach((item: any, idx: number) => {
              newIncidents.push({
                id: `lta-traffic-${idx}`,
                category: 'traffic',
                severity: item.Type === 'Accident' ? 'critical' : 'warning',
                title: `${item.Type || 'Traffic Event'}: ${item.Message?.split('.')[0] || 'Expressway Update'}`,
                detail: item.Message || 'Traffic incident reported via LTA EMAS monitoring.',
                affectedCorridor: 'LTA EMAS Corridor',
                impactBadge: item.Type || 'Live Incident',
                time: 'Live',
              });
            });
          }
        ...
        if (trainRes && trainRes.ok) {
          ...
          if (trainData.value && Array.isArray(trainData.value) && trainData.value.length > 0) {
            trainData.value.forEach((item: any, idx: number) => {
              newIncidents.push({
                id: `lta-train-${idx}`,
                category: 'rail',
                severity: item.Status === 2 ? 'critical' : 'warning',
                title: `Train Service Advisory: ${item.Line || 'MRT Network'}`,
                detail: item.Message || 'Train service update from SMRT/SBST OCC.',
                affectedCorridor: item.Line || 'Islandwide Rail',
                impactBadge: item.Status === 1 ? 'Normal Service' : 'Service Disruption',
                time: 'Live',
              });
            });
          }
        ...
        // Fallback to default realistic incidents
      }
    }

    loadLtaLiveIncidents();
  }, []);
  ...
  }, [isPaused, isExpanded, isDismissed, incidents.length]);
    ...
    return (
      <div className="bg-[#f1ecf2] border-b border-[#c1c6d3] px-4 py-1 flex items-center justify-between text-xs text-[#414751]">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-[#1c1b1f]">{incidents.length} active transit & weather advisories</span>
        </div>
        <button
  ...
  const current = incidents[currentIndex] || DEFAULT_INCIDENTS[0];
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <div className="flex items-center gap-1.5 bg-[#313034] px-2.5 py-1 rounded-full border border-white/10 font-mono text-[11px] font-bold">
            ...
            <span className="text-white tracking-wide">
              {isLiveFromApi ? 'LTA DATA FEED' : 'LIVE DISRUPTIONS'}
            </span>
        ...
        <div className="bg-[#262529] border-t border-white/10 p-4 md:px-8 max-w-[1440px] mx-auto animate-in slide-in-from-top-2 duration-200">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-white/50 font-mono">
                {isLiveFromApi ? '• Connected to LTA DataMall Telemetry' : '• Updated directly from LTA & SMRT OCC'}
              </span>
            </div>
            <button
              onClick={() => onOpenAlertsTab?.()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {incidents.map((inc, idx) => {
              const theme = categoryStyles[inc.category] || categoryStyles.advisory;
              return (
                <div
                  key={inc.id}
                  onClick={() => {
