...
import { DEFAULT_ROUTES, SAVED_ROUTES, ACTIVE_ALERTS } from './data/mockTransitData';
...
export default function App() {
  ...
  const unreadAlertsCount = ACTIVE_ALERTS.filter((a) => a.type === 'warning').length;
      ...
      <main className="flex-grow flex flex-col w-full overflow-hidden pb-16 md:pb-0">
        {activeTab === 'plan' && (
          <div className="flex-grow flex flex-col md:flex-row w-full max-w-[1440px] mx-auto overflow-hidden">
            {/* Left Panel: Search & Filters (4 cols / 33.333% width) */}
            <TripPlannerPanel
              origin={origin}
            ...
            {/* Right Panel: Suggested Routes Results (8 cols / 66.666% width) */}
            <SuggestedRoutesList
              routes={routes}
        ...
        {activeTab === 'live_status' && <LiveStatusView />}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 border border-[#c1c6d3] shadow-2xl flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex justify-between items-start border-b border-[#c1c6d3] pb-3">
              ...
              <button
                ...
                className="text-[#727783] hover:text-[#1c1b1f] p-1 rounded-lg"
              >
